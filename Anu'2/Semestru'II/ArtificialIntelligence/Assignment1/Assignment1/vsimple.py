    ...
    def moveDSF(self, detectedMap):
        ...
        if self.x!= 0 and detectedMap.surface[self.x - 1][self.y] == 0 and (self.x - 1, self.y) not in self.visited:
            if self.x > 0:
                ...
                self.x -= 1
                return
        if self.x!=19 and detectedMap.surface[self.x + 1][self.y] == 0 and (self.x + 1, self.y) not in self.visited:
            if self.x < 19:
                self.moves.append((self.x, self.y))
                self.x += 1
                return
        if self.y!= 19 and detectedMap.surface[self.x][self.y + 1] == 0 and (self.x, self.y + 1) not in self.visited:
            if self.y < 19:
                ...
                self.y += 1
                return
        if self.y!=0 and detectedMap.surface[self.x][self.y - 1] == 0 and (self.x, self.y - 1) not in self.visited:
            if self.y > 0:
                self.moves.append((self.x, self.y))
                self.y -= 1
                return
